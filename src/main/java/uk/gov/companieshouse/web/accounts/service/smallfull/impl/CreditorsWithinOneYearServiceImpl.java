...
public class CreditorsWithinOneYearServiceImpl implements CreditorsWithinOneYearService {
  ...
  private static final String INVALID_URI_MESSAGE =
      "Invalid URI for creditors within one year resource";
  ...
  public List<ValidationError> submitCreditorsWithinOneYear(String transactionId,
      String companyAccountsId, CreditorsWithinOneYear creditorsWithinOneYear, String companyNumber)
    ...
    try {
      if (!creditorsWithinOneYearResourceExists) {
        apiClient.smallFull().creditorsWithinOneYear().create(uri, creditorsWithinOneYearApi)
            .execute();
      } else {
        apiClient.smallFull().creditorsWithinOneYear().update(uri, creditorsWithinOneYearApi)
            .execute();
      }
      ...
      if (e.getStatusCode() == HttpStatus.BAD_REQUEST.value()) {
        ...
        if (validationErrors.isEmpty()) {
          throw new ServiceException(
              "Bad request when creating creditors within one year resource", e);
        }
