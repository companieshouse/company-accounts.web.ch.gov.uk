...
public class ValidationContext {
    ...
    public List<ValidationError> getValidationErrors(List<ApiError> apiErrors) {
        ...
        return apiErrors.stream().map(apiError -> {
            ...
            ValidationMessage validationMessage = ValidationMessage.getMessageForApiError(
                    apiError.getError());
    ...
    private void scanClassValidationMappings(Class<?> clazz, String basePath) {
        ...
        if (depth >= MAX_DEPTH) {
            throw new IllegalStateException(
                    "Maximum recursion depth reached when performing validation scan of class "
                            + clazz.toString());
        }
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            String webPath = basePath.isEmpty() ? field.getName()
                    : basePath + PATH_DELIMITER + field.getName();
            ...
            Annotation validationParentMappingAnnotation = field.getAnnotation(
                    ValidationParentMapping.class);
     ...
     * @return {@code true} if the class is not a primitive, boxed primitive, collection or
     * {@code String}, otherwise {@code false}
     */
